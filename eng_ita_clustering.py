...
def preprocessing():
    ...
    eng_jpn_sentences = list()
    baseline_embeddings = list()
    eng_embeddings = list()
    eng_jpn_embeddings = list()
    eng_ita_embeddings = list()

    # Keep only the eng_jpn_sentences that have japanese translation
    with open(join(TATOEBA_DIR, ENG_JPN_LINKS), 'r') as file:
        ...
        for count, row in enumerate(reader):
            ...
            if eng_sent is not None:
                eng_jpn_sentences.append((eng_sent, jpn_sent))
    ...
    with open(join(SOM_DIR, 'eng_jpn_sentences.pickle'), 'wb') as handle:
        pickle.dump(eng_jpn_sentences, handle, protocol=pickle.HIGHEST_PROTOCOL)
