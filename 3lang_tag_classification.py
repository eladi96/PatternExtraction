...
def latex_summary():
    ...
    with doc.create(Section('Italian Recurrent Model')):
        doc.append('This time we apply the same recurrent architecture to the italian translation of the sentences.')
        with doc.create(Figure(position='h!')) as graph:
            graph.add_image(join('graph', 'ita_model_graph.png'), width='200px')
            graph.add_caption('italian model architecture.')
        values = avg_results(join(MODELS_DIR, 'summary', 'ita_model_summary.txt'))
        doc.append(Command('begin', 'center'))
        with doc.create(Tabular('l||r r r r', pos='centered')) as table:
            table.add_row(("Set", "Loss", "Accuracy", "Recall", "Precision"))
            table.add_hline()
    ...
    with doc.create(Section('Eng-Jpn Model')):
        ...
        with doc.create(Figure(position='h!')) as graph:
            graph.add_image(join('graph', 'eng_jpn_model_graph.png'), width='350px')
            graph.add_caption('Eng-Jpn model architecture.')
        values = avg_results(join(MODELS_DIR, 'summary', 'eng_jpn_model_summary.txt'))
        doc.append(Command('begin', 'center'))
        with doc.create(Tabular('l||r r r r', pos='centered')) as table:
            table.add_row(("Set", "Loss", "Accuracy", "Recall", "Precision"))
            table.add_hline()
    ...
    with doc.create(Section('Eng-Ita Model')):
        doc.append('Same as before but using Italian as second language.')
        with doc.create(Figure(position='h!')) as graph:
            graph.add_image(join('graph', 'eng_ita_model_graph.png'), width='350px')
            graph.add_caption('Eng-Ita model architecture.')
        values = avg_results(join(MODELS_DIR, 'summary', 'eng_ita_model_summary.txt'))
        doc.append(Command('begin', 'center'))
        with doc.create(Tabular('l||r r r r', pos='centered')) as table:
            table.add_row(("Set", "Loss", "Accuracy", "Recall", "Precision"))
            table.add_hline()
...
def main():
    ...
    if not os.path.exists(os.path.join("models", ENG_TOKENIZER)):
        ...
        with open(os.path.join("models", ENG_TOKENIZER), 'rb') as handle:
            print("Loading eng tokenizer...", end=" ")
            eng_tokenizer = pickle.load(handle)
            print("Done.")

    if not os.path.exists(os.path.join("models", JPN_TOKENIZER)):
        ...
        with open(os.path.join("models", JPN_TOKENIZER), 'rb') as handle:
            print("Loading jpn tokenizer...", end=" ")
            jpn_tokenizer = pickle.load(handle)
            print("Done.")

    if not os.path.exists(os.path.join(MODELS_DIR, ITA_TOKENIZER)):
        ...
        del sentences
        print("Done.")
        with open(os.path.join(MODELS_DIR, ITA_TOKENIZER), 'wb') as handle:
            pickle.dump(ita_tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)
            print("Tokenizer saved to " + os.path.join(MODELS_DIR, ITA_TOKENIZER))
    else:
        with open(os.path.join(MODELS_DIR, ITA_TOKENIZER), 'rb') as handle:
    ...
    # ITALIAN MODEL
    ita_dropout = layers.Dropout(0.5, name='dropout')(ita_recurrent)
    ita_output = layers.Dense(NUM_LABELS, activation='softmax', name='classification')(ita_dropout)
    ita_model = Model(ita_input, ita_output, name='ita_model')
    train_model(ita_model, train_x_ita, train_y, valid_x_ita, valid_y)
    del ita_model
    ...
    eng_model = load_model(join(MODELS_DIR, 'eng_model.h5'), custom_objects={'Recall': Recall, 'Precision': Precision})
    ita_model = load_model(join(MODELS_DIR, 'ita_model.h5'), custom_objects={'Recall': Recall, 'Precision': Precision})
    comb_model.get_layer('eng_recurrent').set_weights(eng_model.get_layer('eng_recurrent').get_weights())
    comb_model.get_layer('eng_recurrent').trainable = False
    comb_model.get_layer('ita_recurrent').set_weights(ita_model.get_layer('ita_recurrent').get_weights())
    comb_model.get_layer('ita_recurrent').trainable = False
    del eng_model
    del ita_model
    ...
    with open(join(MODELS_DIR, 'summary', 'ita_model_summary.txt'), mode='a') as file:
        evaluate_model('ita_model', file, train_x_ita, train_y, valid_x_ita, valid_y, test_x_ita, test_y)
    with open(join(MODELS_DIR, 'summary', 'eng_jpn_model_summary.txt'), mode='a') as file:
    ...
    with open(join(MODELS_DIR, 'summary', 'eng_ita_model_summary.txt'), mode='a') as file:
